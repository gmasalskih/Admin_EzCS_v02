...
class ServiceProvider(
    ...
    suspend fun <T : Entity> retrieveEntities(collectionName: String, clazz: KClass<T>): List<T> {
        val listEntities = firestore.getCollectionItems(collectionName, clazz.java).map { entity ->
            entity::class.declaredMemberProperties.forEach { prop ->
                if (prop.annotations.find { it is ContentType } != null) {
                    when (val content = prop.call(entity)) {
                        is List<*> -> {
                            prop.javaField?.let { field ->
                                field.isAccessible = true
                                val newContentList = content.filterIsInstance<String>()
                                    .map { fileName -> dropbox.getFileUrl(entity.contentsPath(), fileName) }
                                    .toList()
                                if (newContentList.isEmpty()) throw  Exception("Entity ${entity.name} have field ${prop.name}, but it's field have empty list $content")
                                field.set(entity, newContentList)
                            }
                        }
                        is String -> {
                            prop.javaField?.let { field ->
                                field.isAccessible = true
                                field.set(entity, dropbox.getFileUrl(entity.contentsPath(), content))
                            }
                        }
                        else -> {
                            throw Exception("Entity ${entity.name} have field ${prop.name} contains incompatible type of ${prop.returnType}")
